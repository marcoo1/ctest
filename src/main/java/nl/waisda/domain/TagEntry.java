...
public int recomputeScore() {
		...
		/** Recomputes the score from the current match and bonus fields. */
	public int recomputeScoreLocation() {
		if (tag.length() < 2) {
			return 0;
		}
	...
	public void updateScoreLocation() {
		setScore(recomputeScoreLocation());
	}

	/** Updates this entry's score field using recomputeScore. */
	public void updateScore() {
		String gameTypeId = getGameType();
		if (gameTypeId.equals("location")) {
			setScore(recomputeScoreLocation());
		} else if (gameTypeId.equals("normal")) {
    ...
    public String toString() {
		return String.format(
				"#%d normalizedTag:%s user:%d video:%d game:%d time:%d gameType:%s",
				getId(), getNormalizedTag(), getOwner().getId(), getGame()
						.getVideo().getId(), getGame().getId(), getGameTime(), getGameType());
	}
